...
class ActivationsAndGradients:
    ...
    def save_activation(self, module, input, output):
        ...
        # print(f"activation shape(模型輸出): {activation.shape}")
    ...
    def save_gradient(self, module, grad_input, grad_output):
        ...
        # print(f"grad shape(反向傳播): {grad.shape}")
...
class GradCAM:
    ...
    def scale_cam_image(cam, target_size=None):
        ...
        # print(f"target sizere: {target_size}")
        print(f"result shapere: {result.shape}")
    ...
    def __call__(self, input_tensor, target_category=None):
        ...
        # print(f"cam_per_target_layer shape: {cam_per_layer[0].shape}")
...
class GradCAMPlusPlus(GradCAM):
    ...
    def get_cam_weights(grads, activations):
        ...
        sum_activations = np.sum(activations, axis=(2, 3, 4), keepdims=True)  # shape: [B, C, 1, 1, 1]
        eps = 1e-8

        alpha_num = grads_power_2
        alpha_denom = 2 * grads_power_2 + sum_activations * grads_power_3
        alpha_denom = np.where(alpha_denom != 0.0, alpha_denom, eps)  # avoid divide-by-zero

        alpha = alpha_num / alpha_denom  # shape: [B, C, D, H, W]
        relu_grads = np.maximum(grads, 0)

        weights = np.sum(alpha * relu_grads, axis=(2, 3, 4), keepdims=True)  # shape: [B, C, 1, 1, 1]
