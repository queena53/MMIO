...
from data_slice91 import get_folds_slice91
...
class OneCycleLRScheduler(_LRScheduler):
    ...
    """
    def __init__(self, optimizer, max_lr, total_epochs, pct_start=0.0, min_lr=1e-6, last_epoch=-1):
        self.max_lr = max_lr
...
def train_one_epoch(model, dataloader, optimizer, criterion, device):
    ...
    for batch_idx, (x, y) in enumerate(tqdm(dataloader, desc="Training", leave=False)):
        ...
        # # Debug: shape & label check
        # if batch_idx == 0:
        #     print(f"[Debug] Input shape: {x.shape}, Label shape: {y.shape}, Unique labels: {torch.unique(y)}")
        ...
        # loss += l1_lambda * l1_loss  # 添加 L1 正则化项
...
def validate(model, dataloader, criterion, device,use_wandb=False, class_names=None, epoch=None):
    model.eval()
    total_loss, correct, total = 0, 0, 0
    all_preds = []
    all_labels = []
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(tqdm(dataloader, desc="Validation", leave=False)):
            x, y = x.to(device), y.to(device)
            out = model(x)
            loss = criterion(out, y)
            ...
            # Debug: 顯示模型預測 & 機率
            # if batch_idx == 0:
            #     print(f"[Debug] Model output logits:\n{out[:3]}")
            #     print(f"[Debug] Predicted: {out.argmax(1)[:3]}, Ground truth: {y[:3]}")
            #     probs = torch.softmax(out, dim=1)
            #     print(f"[Debug] Probabilities: {probs[:3]}")
...
def train_all_folds(config):
    ...
    folds = get_folds_slice91(config)
    ...
    for fold_idx, fold in enumerate(folds):
        ...
        if config["use_wandb"]:
            wandb.init(project=config["project_name"], name=f"desnet201_FocalLoss_dropout_fold_{fold_idx}", config=config)
        ...
        # n_input_channels=120,    # 輸入通道數為 120
        # num_classes=2            # 二分類
        # ).to(device) 
        ...
        scheduler = OneCycleLRScheduler(optimizer, max_lr=4e-5, total_epochs=config["epochs"], pct_start=0.15, min_lr=1e-6)
